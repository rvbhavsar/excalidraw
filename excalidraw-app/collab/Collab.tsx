...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  constructor(props: CollabProps) {
    ...
    this.fileManager = new FileManager({
      ...
      // files travel inline with the drawing's `files` JSONB column on every
      // saveDrawing() call (see syncElements/queueSaveToBackend below), so
      // fetching just means re-reading the drawing record.
      ...
      // no-op: the next saveDrawing() call persists these files inline
  ...
  private beforeUnload = withBatchedUpdates((event: BeforeUnloadEvent) => {
    ...
    if (
      ...
      (this.fileManager.shouldPreventUnload(syncableElements) ||
        !isSavedToBackend(this.portal.roomId, syncableElements))
    ) {
  ...
  saveCollabRoomToBackend = async (
    ...
    try {
      const stored = await saveDrawing(
        roomId,
        syncableElements,
  ...
  startCollaboration = async (
    existingRoomLinkData: null | { roomId: string },
  ) => {
    ...
    try {
      ...
      this.portal.socket = this.portal.open(
        socketIOClient(import.meta.env.VITE_APP_API_URL, {
          transports: ["websocket", "polling"],
          auth: { token, username: this.state.username },
        }),
        roomId,
        // roomKey is now unused (kept only to satisfy Portal.isOpen()'s truthy check)
        "n/a",
      );
        ...
        switch (data.type) {
          ...
          case WS_SUBTYPES.INIT: {
            if (!this.portal.socketInitialized) {
              ...
              // noop if already resolved via init from the backend
  ...
  private initializeRoom = async ({
    ...
    if (fetchScene && roomLinkData && this.portal.socket) {
      ...
      try {
        const drawing = await getDrawing(roomLinkData.roomId);
